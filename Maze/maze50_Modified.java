...
         * obstacles or move the robot and/or target.
        ...
        private class ActionHandler implements ActionListener {
            ...
            public void actionPerformed(ActionEvent evt) {
                ...
                if (cmd.equals("Clear")) {
                    ...
                    bidirectional.setEnabled(false);
                ...
                } else if (cmd.equals("Save Maze")) {
                	saveToTxt();
                } else if (cmd.equals("Load Maze")) {
                	LoadMaze();
                }
            }
        ...
        // basic buttons
        JButton resetButton, mazeButton, clearButton, realTimeButton, stepButton, animationButton, saveMaze, loadMaze;
        
        // buttons for selecting the algorithm
        JRadioButton dfs, bfs, aStar, greedy, dijkstra, bidirectional;
        ...
        public MazePanel(int width, int height) {
            ...
            saveMaze.addActionListener(new ActionHandler());
            saveMaze.setBackground(Color.lightGray);
            saveMaze.setToolTipText
                    ("Saves the current state of maze in a .txt file");
            
            loadMaze = new JButton("Load Maze");
            loadMaze.addActionListener(new ActionHandler());
            loadMaze.setBackground(Color.lightGray);
            loadMaze.setToolTipText
            ...
            bidirectional = new JRadioButton("bidirectional");
            bidirectional.setToolTipText("bidirectional algorithm");
            algoGroup.add(bidirectional);
            bidirectional.addActionListener(new ActionHandler());
            ...
            greedy.setBounds(600, 325, 70, 25);
            dijkstra.setBounds(530, 350, 70, 25);
            bidirectional.setBounds(600, 350, 80, 25);
            ...
            saveMaze.setBounds(520, 437, 80, 25);
            loadMaze.setBounds(610, 437, 80, 25);
    ...
    static protected JSpinner addLabeledSpinner(Container c,
        ...
        private void expandNode(){
            ...
            if (dijkstra.isSelected()){
                ...
                Cell current;
                if (dfs.isSelected() || bfs.isSelected() || bidirectional.isSelected()) {
                    // Here is the 3rd step of the algorithms DFS and BFS
                    // 3. Remove the first state, Si, from OPEN SET ...
                	current = openSet.remove(0);
                	if(bidirectional.isSelected() && current == robotStart) {
                		System.out.println("Bidirectional is selected - target is added to the openset");
                    	openSet.add(targetPos);
                	}
                } else {
                ...
                if (current.row == targetPos.row && current.col == targetPos.col && !(bidirectional.isSelected())) {
                    // ... then terminate etc
                    Cell last = targetPos;
                ...
                if (bidirectional.isSelected()) {
                	int r = current.row;
                	int c = current.col;
                	int[] rowpos = {r, r, r-1, r+1};
                	int[] colpos = {c+1, c-1, c, c};
                	for (int i = 0; i < 4; i++) {
                		if (grid[rowpos[i]][colpos[i]] == FRONTIER) {
                			System.out.println("Frontier intersection...........................................................................................................");
                			System.out.println("	Frontier row: "+rowpos[i]+"	Frontier col: "+colpos[i]);
                			searching = false;
                			found = true;
                			endOfSearch = true;
                			for (int j = 0; j < openSet.size(); j++) {
                				if (openSet.get(j).row == rowpos[i] && openSet.get(j).col == colpos[i]) {
                					System.out.println("	openSet row: "+openSet.get(j).row+"	openSet col: "+openSet.get(j).col);
                					plotRoute(openSet.get(j), current);
                					return;
                				}
                			}
                		}
        		...
        		System.out.println("expanded: "+expanded+"	-	No of successors of cell at ( "+current.row+", "+current.col+" ) - "+succesors.size());
                ...
                succesors.stream().forEach((cell) -> {
                    ...
                    } else if (bfs.isSelected() || bidirectional.isSelected()){
...
         */
        private void plotRoute(){
        	System.out.println("In PlotRoute");
            searching = false;
            endOfSearch = true;
            int steps = 0;
            double distance = 0;
        ...
        private void plotRoute(Cell f1, Cell f2) {
        	System.out.println("Inside plotRoute for Bidirectional");
        	if (endOfSearch == true) {
                int steps = 0;
                double distance = 0;
                grid[f1.row][f1.col]=ROUTE;
                grid[f2.row][f2.col]=ROUTE;
                while (true) {
                    steps++;
                    if (diagonal.isSelected()) {
                        int dx = f1.col-f1.prev.col;
                        int dy = f1.row-f1.prev.row;
                        distance += Math.sqrt(dx*dx + dy*dy);
                    } else { 
                        distance++;
                    }
                    f1 = f1.prev;
                    grid[f1.row][f1.col] = ROUTE;
                    if (f1.row == robotStart.row && f1.col == robotStart.col || f2.row == targetPos.row && f2.col == targetPos.col) {
                    	break;
                    }
                } 
                while (true) {
                    steps++;
                    if (diagonal.isSelected()) {
                        int dx = f2.col-f2.prev.col;
                        int dy = f2.row-f2.prev.row;
                        distance += Math.sqrt(dx*dx + dy*dy);
                    } else { 
                        distance++;
                    }
                    f2 = f2.prev;
                    grid[f2.row][f2.col] = ROUTE;
                    if (f2.row == robotStart.row && f2.col == robotStart.col || f2.row == targetPos.row && f2.col == targetPos.col) {
                    	break;
                    }
                } 
        		grid[robotStart.row][robotStart.col]=ROBOT;
...
         */
        private void fillGrid() {
        	System.out.println("Inside fillGrid function");
...
          */
        ...
        public void saveToTxt() {
        	try {
        	      FileWriter myWriter = new FileWriter("C:\\Users\\harsh\\Documents\\UTD\\Sem 3\\AI\\Assignments\\A1\\maze52\\savemaze.txt");
        	      for (int r = 0; r < rows; r++) {
                      for (int c = 0; c < columns; c++) {
                          if (grid[r][c] == EMPTY) {
                        	  myWriter.write(" ");
        ...
        public void LoadMaze() {
        	try { 
        		FileReader fr=new FileReader("C:\\Users\\harsh\\Documents\\UTD\\Sem 3\\AI\\Assignments\\A1\\maze52\\savemaze.txt");
        		char symbol; 
        		...
        		for (int r = 0; r < rows; r++) {
        			...
        			for (int c = 0; c < columns; c++) {
        				symbol = (char)fr.read();
        				System.out.print("symbol at ("+r+","+c+"): "+symbol);
        				if (symbol == ' ') {
